        ...
        while (menuSelection != 0) {
            ...
            try {
                if (menuSelection == 1) {
                    ...
                    String credentials = consoleService.promptForLogin();	// Use the ConsoleService to get username/password
                    if( credentials.split(",").length == 2 ) {
                    	...
                    	//				typically a Map is used to hold the response
                        ...
                        if (response.hasBody()) {										// If the response has a body (is there any data in the response?)
                            String token = (String) response.getBody().get("token");	// get the value from 'token' attribute and store it.
                            hotelService.AUTH_TOKEN = token;							// Store the token in the HotelService variable to hold it
                            System.out.println("Login Successful");						// Tell the user that the login was successful1
                        }
                    } else {
