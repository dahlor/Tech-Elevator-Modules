...
select sum(visitors) as total_visitors_for_all_parks
from park
;

-- select the average number of visitors for all parks

select round(avg(visitors)) as average_visitors_for_all_parks
from park
;
...
select count(site_id) as number_of_sites_with_utilities_hook_up
from site
where utilities = 'true'
;
...
select count(reservation_id) as total_number_of_reservations
from reservation
;
