...
select name, continent, gnp, lifeexpectancy from country where lifeexpectancy >= 70 and gnp between 10 and 100;

-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)

select round ((gnp/population) * 10000000, 2) as Per_Capita_GNP_in_USD_in_2000_for_some_reason, name from country where continent = 'Europe'; 

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)

select name, (2021 - indepyear) as years_since_independence from country where indepyear is not null;